...
////////////////////////////////////////////////////////////
template <typename T>
sf::Rect<T>::Rect(const sf::Vector2<T>& position, const Vector2<T>& size) :
	left(position.x),
	top(position.y),
	width(size.x),
	height(size.y)
...
bool sf::Rect<T>::intersects(const sf::Rect<T>& rectangle, sf::Rect<T>& intersection) const
{
	...
	{
		intersection = sf::Rect<T>(interLeft, interTop, interRight - interLeft, interBottom - interTop);
		return true;
	}
	else
	{
		intersection = sf::Rect<T>(0, 0, 0, 0);
		return false;
	}
}
